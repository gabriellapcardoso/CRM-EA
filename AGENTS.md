...
- **Knowing the comment-stripping rule did not prevent it happening again.** The rule above was written on 2026-09-02; on 2026-09-03 a new static test passed with the regression injected, matching a string in the comment written beside the fixed line. The assertion looks equally correct in both versions and the suite is green either way. Regression injection is what catches it, and it depends on no one remembering anything — do it on every static test
...
## Reading Rules
- **A capability with no call site is indistinguishable from a missing capability.** Three of them shipped broken on 2026-09-03: `configureWebhook()` (5 weeks of a silent WhatsApp channel), `INTERNAL_API_SECRET` (an env var name only the code knew — the AI agent was never triggered by an inbound message) and `useRetryMessage()` (failed messages with no retry button). All three pass review, have correct types and a comment stating their purpose; one even had a unit test of the method itself. None of that answers **who calls it**. Before assuming something runs, `grep` its name excluding the file that defines it. For env vars, also check the environment (`supabase secrets list`, Vercel dashboard) — not just the repo
- **An enumeration asserts completeness.** "The four keys that silence the AI" is not a partial list, it is a claim that there are four. A reader who checks all four concludes the problem is elsewhere and stops looking. This exact doc was published with four and was wrong two hours later — the fifth (`board_ai_config.agent_mode`) sat in an early-return the list never swept. Derive closed lists from the code path, not from what you just finished debugging; if you cannot sweep it, write "among others" and do not number it
- **When a symptom survives fixing its confirmed cause, look for the next gate — not for a flaw in the fix.** Guards in series only reveal the second one after the first stops firing. On 2026-09-03 "the agent does not reply" was correctly diagnosed and fixed three times in one night (`ai_enabled`, `ai_takeover_enabled`, `agent_mode`), and the symptom was identical after the first two. Reading the route's log each round is what made it three quick rounds instead of three rounds of guessing
- **A config screen that changes behaviour on open-and-save is a trap.** `board_ai_config.agent_mode` defaults to `observe`, and the row only exists once someone saves that screen. No row means the agent sends; a freshly created row means it goes silent. Nobody edited a field — merely visiting and saving flipped it. Prefer defaults whose failure is loud; when a silent default is right, the screen must state what the mode does, not just name it
- **A vendor's aggregate status answers the easy question, not the one that matters.** Evolution reported `connectionStatus: open` for six weeks on an instance that had no WhatsApp behind it at all. Look for the field that only exists when the thing actually works: an account identifier, a non-zero count, a last-used timestamp
- **An error message that asserts an unverified cause becomes a false root cause later.** `"QR code not available. Instance may already be connected."` was a guess; a month later it was read as a diagnosis and became the justification for a code branch that re-marked channels as connected. Errors state what was observed — `"field X missing (fields received: a, b, c)"` — never what might have caused it
- **Empty satisfies checks written for values.** A masking rule for this already existed (`left(v,6) || '...'` turned an empty `ownerJid` into `"..."`, read as "redacted" for a day). On 2026-09-04 it recurred in a *comparison*: a deploy watcher looped `until [ "$now" != "$before" ]`, the extraction regex stopped matching after the asset name changed shape, `$now` became empty, and `"" != "0tlqm..."` fired — announcing "deployed" with no evidence at all. The rule is not about masking; it is that an empty value passes almost any check designed to distinguish two values. Assert what you actually require (non-empty **and** different), and emit `(empty)` / `(null)` / `(N chars)` rather than a value-shaped placeholder
- **A reproduction that does not reproduce is not evidence of absence.** The entry path is part of the case. Twice on 2026-09-04 a first repro came back clean and nearly closed a real bug: the message-bubble collapse would not reproduce in a standalone page because Tailwind was not loaded, and the frozen-URL bug does not reproduce when `/messaging` is opened without `?id=` — only when the page loads with the param already there, which is how a person arrives from a link. Before concluding "cannot reproduce", check that you entered the way the user entered: same build (production vs dev), same stylesheets, same URL shape, same starting state
- **A conclusion drawn from a broken environment contaminates the healthy one.** `?number=` was documented as mandatory because without it the server "returns an empty body" — true of the misconfigured instance, which returned empty for everything. On a real instance that same param suppresses the QR entirely. When recording observed behaviour, record *what it was observed against*, not just what was seen
...
- **`stage_ai_config` gates the agent per stage, and `system_prompt` is NOT NULL with no default.** Enabling a stage means deciding what the AI says to a real customer — it is a product decision, not a toggle. With no enabled row for the deal's stage, the agent exits with "AI not enabled for this stage": the message lands, contact and deal are created, and nobody replies. `provision-stages` picks its template by the stage's **position** in the ordered list, never by the raw `"order"` value — boards here start at `order: 1`, which made the first-contact template unreachable
- **The kill switch, not HITL, is what stops a message going out.** Every agent reply goes through `ChannelRouterService.sendMessage()` → `checkWhatsAppSendGuard()`. `whatsapp_kill_switch_active` blocks *all* WhatsApp sending, manual inbox replies included. HITL governs stage advancement only. Verify a safety layer with real traffic before trusting it: this one was unexercised code until it blocked three real replies
...
- **When you force a width to sweep an invariant, confirm the width actually changed.** Setting `el.style.width` on a `flex: 1` item does nothing — `flex-grow` wins — so a sweep across 900/700/560/440px returned the identical row width four times and looked like a passing test. Set `flex: none` and `min-width: 0` alongside the width, and print the independent variable in every row of the result so a frozen one is visible
- **A percentage `max-width` needs a containing block with a definite width.** `.message__bubble` had `max-width: 62%` two wrappers deep inside boxes that were all shrink-to-fit, so the percentage referred to a box whose width depended on the bubble. Browsers resolve that circular reference near zero, and `overflow-wrap` then breaks mid-word — the signature is every bubble at the same tiny width with one character per line. Put the limit on an element whose parent declares `width: 100%`
