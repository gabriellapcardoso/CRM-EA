...
function textoOuNulo(v: unknown): boolean {
...
export function validarPayloadProspeccao(bruto: unknown): ResultadoValidacao {
  ...
  if (demo.tipo === "link" && !texto(demo.link)) return { ok: false, erro: "demo.link obrigatório quando tipo=link" };

  if (!texto(p.mensagem_whatsapp)) return { ok: false, erro: "mensagem_whatsapp obrigatória" };
