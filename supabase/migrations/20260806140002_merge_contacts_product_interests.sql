-- Ensina merge_contacts a mover contact_product_interests do contato duplicado
-- pro contato alvo. Sem isso, mesclar dois contatos deixa os interesses de
-- produto do duplicado órfãos (apontando pra um contact_id que sai de cena).
--
-- Baseado na versão vigente de merge_contacts (20260406120000), não na
-- migration original de fevereiro — editar essa não afetaria bancos onde a
-- função já foi substituída pela correção de voice_calls.
--
-- Interesses PENDENTES (converted_at IS NULL) duplicados no mesmo produto:
-- mantém o do contato alvo, descarta o do duplicado (ON CONFLICT DO NOTHING).
-- Interesses já CONVERTIDOS (histórico) sempre são preservados, sem conflito.
...
BEGIN
  ...
  -- 11. Mover contact_product_interests: pendente duplicado no mesmo produto
  -- mantém o do target (ON CONFLICT DO NOTHING descarta o do source);
  -- histórico convertido é sempre preservado (fora do índice único parcial).
  INSERT INTO contact_product_interests
    (contact_id, product_id, observacao, converted_at, converted_deal_id, organization_id)
  SELECT p_target_id, product_id, observacao, converted_at, converted_deal_id, organization_id
  FROM contact_product_interests
  WHERE contact_id = p_source_id
  ON CONFLICT (contact_id, product_id) WHERE converted_at IS NULL DO NOTHING;
  GET DIAGNOSTICS v_count = ROW_COUNT;
  v_moved := v_moved || jsonb_build_object('contact_product_interests', v_count);

  DELETE FROM contact_product_interests WHERE contact_id = p_source_id;
