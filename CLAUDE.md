...
**Cuidado ao cancelar/invalidar com key estreita**: se o `onMutate` também escreve otimisticamente em `entity.detail(id)` via `setQueryData`, `.lists()`/`entityCachesExceptDetail()` **não cobrem esse cache** (ao contrário de `.all`, que cobria por prefix-match). Sem um `cancelQueries({ queryKey: entity.detail(id) })` adicional, um fetch de detail em andamento pode sobrescrever a escrita otimista depois — race condition real, já achada e corrigida em `useUpdateDeal`/`useMoveDeal`/`useUpdateConversation`/`useUpdateContact` (ver `DESAFIOS.md`). O `useUpdateContact` entrou nessa lista em 2026-09-05 e mostra o gatilho geral: **a race nasce no dia em que alguém passa a LER `entity.detail(id)`**, não no dia em que a mutation foi escrita. Ao criar tela de detalhe nova, conferir se a mutation da entidade alcança esse cache.
...
**Cockpit do deal — coluna única (2026-08-31)**: a tela era um grid de 3 painéis com 3 rolagens e `min-width: 1180px`. Virou uma coluna com uma rolagem só. Os painéis (`.cockpit__aside`, `.cockpit__center`) são `display: contents` — **regra de layout, não de DOM**: eles continuam sendo os filhos diretos de `.cockpit__body` na árvore, então seletor `>` alcança os painéis (sem caixa), não os blocos. Usar descendente. A ordem das seções vive nas classes `.cockpit__sec--contato|risco|hitl|passos|historico|assistente|ref` (`order: 1..7`, revisão de 2026-09-04); bloco sem classe cai em `order: 0` e vai pro topo da tela sem avisar. **Uma classe por bloco, nunca uma classe para vários**: `--decidir` cobria risco, HITL e próximos passos ao mesmo tempo, e blocos empatados no `order` caem na ordem do DOM — que punha a decisão antes do risco que a justifica. Os dados do deal foram absorvidos pela `.field-grid` do bloco de contato, então `--deal` não existe mais. Guarda: `test/cockpitLayout.test.ts` (inclui uma asserção de que `--decidir` não volta).

**Layout não é testável em happy-dom** (sem engine de layout: `getBoundingClientRect()` devolve zero, `scrollWidth === clientWidth` sempre). Teste de "não estoura" renderizando componente é falso-positivo. Afirmar invariantes do CSS como texto e medir no browser. Ao medir, **conferir que a variável independente mudou**: forçar `width` num item `flex: 1` não muda nada (o `flex-grow` vence), e um sweep de quatro larguras devolve a mesma medida quatro vezes com cara de teste aprovado. Ver `AGENTS.md`, Cockpit / CSS Layout Rules.
...
**Texto de estado nunca entra num `value` de formulário.** A tela mostra "Empresa não vinculada", "Sem Dono", "—", "fora do lote carregado" quando não há dado. Reusar essa MESMA variável pra pré-preencher um campo transforma texto de interface em dado no primeiro salvamento — em 2026-09-05 o modal de edição do contato abria com "Empresa não vinculada" no campo empresa, e ligar a vinculação sem corrigir isso criaria uma empresa com esse nome. Manter duas variáveis separadas: a do valor real (`empresaDoContato?.name`) e a do texto exibido (`companyName`).

**Regra CSS que mira classe de componente pode estar mirando nada.** Este repositório mistura CSS semântico (`app/globals.css`) com componentes Tailwind: `StageBadge` renderiza utilitários e **nunca emite `.badge-stage`**, então `.cell-name__co .badge-stage { flex: none }` não alcançava nada. CSS não erra em seletor sem correspondência — não há warning, o build passa, e um teste que afirma "a regra existe no arquivo" passa também. Conferir no elemento renderizado (`getComputedStyle`: a propriedade pegou?) e preferir alvo por posição (`.pai > :not(.filho-que-encolhe)`) quando o vizinho é de outro vocabulário.

**Voltar é contextual, e o rótulo sai do mesmo lugar que o destino**: `resolverOrigem()` (`lib/navigation/origem.ts`) lê `?from=` da URL. Não dá pra derivar isso do histórico: ele não é legível por script, e `document.referrer` vem vazio em navegação client-side do App Router — o rótulo cairia no genérico justo nos casos que importam. Quem abre um cockpit passa o `from` (`PipelineView`, `DecisionQueuePage`, detalhe do contato).
...
**O que o agente sabe do negócio vem de `board_ai_config.business_context`, não de `products`.** A tabela `products` do CRM existe, está vazia e **não é lida pelo prompt** — preenchê-la não muda nada no que o agente sabe. O campo que o `buildSystemPrompt` interpola (`agent.service.ts:838`) é `business_context`, texto livre por board. A fonte de verdade de serviço e preço da esteira é o **gerador de propostas comerciais** (`catalogo_servicos` + `catalogo_modificadores`, Supabase próprio), não este CRM — se algum dia isso for alimentar o agente, é de lá pra `business_context`, filtrando por `tipo` (aquele catálogo atende também cliente político, fora desta esteira). Ver `TODOS.md`.
