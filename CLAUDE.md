...
**Telas de detalhe são página, não painel (2026-09-04)**: o detalhe do contato era um `.detail-pane` de 340px encostado na lista e agora é `/contacts/[contactId]` em tela cheia — o estado saiu do `useState` e foi pra URL, que dá pra recarregar e mandar pra alguém. `.detail-pane` **continua no CSS porque o inbox ainda usa**; não usar em tela nova. Na mesma revisão a `.table-list` perdeu o `min-width: 840px` (a coluna empresa virou segunda linha do nome, `.cell-name__stack`) e o `.card-approval` trocou `1fr 268px` por `auto-fit`. Piso medido da tabela de contatos: **648px + 40px de padding do `.table-list__scroll`** — cabe em 1280px com a barra lateral, e ainda rola ~28px se a barra E o painel de IA estiverem abertos ao mesmo tempo num monitor de 1280. Guarda: `test/telasDeDetalheSemRolagemLateral.test.ts`.
